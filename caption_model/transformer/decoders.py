...
import numpy as np
from caption_model.transformer.la import Lam
from caption_model.transformer.attention import MultiHeadAttention
from caption_model.transformer.utils import sinusoid_encoding_table, PositionWiseFeedForward
from caption_model.containers import Module, ModuleList
...
class MeshedDecoder(Module):
    def __init__(self, vocab_size, max_len, N_dec, padding_idx, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1,
        ...
        self.label_attention=Lam(d_model,h)
    ...
    def forward(self, input, encoder_output, mask_encoder,labels,probs): #  
        # input (b_s, seq_len)
        b_s, seq_len = input.shape[:2]

        l_number = encoder_output.shape[1]
        dims=encoder_output.shape[-1]
        labels_att=self.label_attention(labels,b_s,l_number,self.word_emb,probs,dims)
        encoder_output_mix=torch.multiply(encoder_output,labels_att)
        mask_queries = (input != self.padding_idx).unsqueeze(-1).float()  # (b_s, seq_len, 1)
        mask_self_attention = torch.triu(torch.ones((seq_len, seq_len), dtype=torch.uint8, device=input.device),
                                         diagonal=1)
        mask_self_attention = mask_self_attention.unsqueeze(0).unsqueeze(0)  # (1, 1, seq_len, seq_len)
        mask_self_attention = mask_self_attention + (input == self.padding_idx).unsqueeze(1).unsqueeze(1).byte()
        mask_self_attention = mask_self_attention.gt(0).bool()  # (b_s, 1, seq_len, seq_len)
        ...
        for i, l in enumerate(self.layers):
            out = l(out, encoder_output_mix, mask_queries, mask_self_attention, mask_encoder)
