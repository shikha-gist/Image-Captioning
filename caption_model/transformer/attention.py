...
class GeomtericAttention(nn.Module):
    '''
    Scaled dot-product attention with memory
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, m):
        ...
        :param m: Number of memory slots
        '''
        super(GeomtericAttention, self).__init__()
        self.fc_q = nn.Linear(d_model, h * d_k)
        self.fc_k = nn.Linear(d_model, h * d_k)
        self.fc_v = nn.Linear(d_model, h * d_v)
        self.fc_o = nn.Linear(h * d_v, d_model)
        self.fc_g = nn.Linear(64, 1)
        #self.fc_g = nn.Linear(96, 1)

        self.m_k = nn.Parameter(torch.FloatTensor(1, m, h * d_k))
        self.m_v = nn.Parameter(torch.FloatTensor(1, m, h * d_v))
        self.m_g = nn.Parameter(torch.FloatTensor(1, m, 1))
    ...
    def init_weights(self):
        ...
        nn.init.xavier_uniform_(self.fc_v.weight)
        nn.init.xavier_uniform_(self.fc_o.weight)
        nn.init.xavier_uniform_(self.fc_g.weight)

        nn.init.normal_(self.m_k, 0, 1 / self.d_k)
        nn.init.normal_(self.m_v, 0, 1 / self.m)
        nn.init.normal_(self.m_g, 0, 1 / self.d_k)
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None,geometric=None):
        '''
        Computes
        :param queries: Queries (b_s, nq, d_model)
        :param keys: Keys (b_s, nk, d_model)
        :param values: Values (b_s, nk, d_model)
        :param geometric: Values (b_s,nk,64)
        :param attention_mask: Mask over attention values (b_s, h, nq, nk). True indicates masking.
        :param attention_weights: Multiplicative weights for attention values (b_s, h, nq, nk).
        :return:
        '''
        ...
        m_k = np.sqrt(self.d_k) * self.m_k.expand(b_s, self.m, self.h * self.d_k)
        m_v = np.sqrt(self.m) * self.m_v.expand(b_s, self.m, self.h * self.d_v)
        m_g = np.sqrt(self.d_k) * self.m_g.expand(b_s, geometric.shape[1],self.m, 1)
        ...
        #expon=torch.exp(att)  ## exponential

        g = torch.cat([self.fc_g(geometric), m_g], 2).permute(0,3,1,2)
        g = F.relu(g) ## new
        
        out= torch.log(torch.clamp(g,min=1e-6))+att ##new
        out= torch.softmax(out,-1)
        out= torch.matmul(out,v).permute(0, 2, 1, 3).contiguous().view(b_s, nq, self.h * self.d_v)
        out=self.fc_o(out)
        #el_mul=torch.mul(g,expon) ## element-wise multiplication

        #weights = torch.softmax(el_mul, -1)
        #out = torch.matmul(weights, v).permute(0, 2, 1, 3).contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
        #out = self.fc_o(out)  # (b_s, nq, d_model)
        return out
...
class MultiHeadAttention(Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None,boxes=None):
        ...
        if self.identity_map_reordering:
            ...
            if boxes is None:
                ...
                out = self.attention(q_norm, k_norm, v_norm, attention_mask, attention_weights,boxes)
            out = queries + self.dropout(torch.relu(out))
        else:
            if boxes is None:
                out = self.attention(queries, keys, values, attention_mask, attention_weights)
            elif boxes is not None:
                out = self.attention(queries, keys, values, attention_mask, attention_weights, boxes)
