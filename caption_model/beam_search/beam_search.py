...
class BeamSearch(object):
    ...
    def _expand_state(self, selected_beam, cur_beam_size):
        def fn(s):
            ...
            s = torch.gather(s.view(*([np.int64(self.b_s), np.int64(cur_beam_size)] + shape[1:])), 1, beam.expand(*([np.int64(self.b_s), np.int64(self.beam_size)] + shape[1:])))
            s = s.view(*([-1, ] + shape[1:]))
            return s
    ...
    def apply(self, visual: utils.TensorOrSequence,boxes:utils.TensorOrSequence,labels:utils.TensorOrSequence,probs:utils.TensorOrSequence, out_size=1, return_probs=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                visual,boxes,labels,probs, outputs = self.iter(t, visual,boxes,labels ,probs,outputs, return_probs, **kwargs)
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence,boxes: utils.TensorOrSequence, labels: utils.TensorOrSequence,probs: utils.TensorOrSequence,outputs, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob = self.model.step(t, self.selected_words, visual,boxes,labels,probs, None, mode='feedback', **kwargs)
        ...
        probs=self._expand_visual(probs, cur_beam_size, selected_beam)
        boxes=self._expand_visual(boxes, cur_beam_size, selected_beam)
        ...
        return visual,boxes,labels,probs, outputs
